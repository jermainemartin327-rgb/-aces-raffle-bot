...
def init_db():
    with db() as conn:
        ...
        # Add persistent raffle-board tracking to databases created by older
        # versions of the bot.
        raffle_columns = {
            row["name"] for row in conn.execute("PRAGMA table_info(raffle)").fetchall()
        }
        if "board_channel_id" not in raffle_columns:
            conn.execute("ALTER TABLE raffle ADD COLUMN board_channel_id INTEGER")
        if "board_message_id" not in raffle_columns:
            conn.execute("ALTER TABLE raffle ADD COLUMN board_message_id INTEGER")
...
def build_board_message():
    ...
    rows = get_slots()
    claimed = sum(1 for row in rows if row["user_id"] is not None)
    bar_length = 20
    filled_blocks = round((claimed / raffle["slot_count"]) * bar_length)
    ...
    header = (
        ...
        f"📊 **Spots:** {progress_bar} {claimed}/{raffle['slot_count']}\n"
        f"🏆 **Winners:** 1\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
    )
    ...
    slot_text = ""
    for row in rows:
        name = "OPEN" if row["user_id"] is None else row["username"]
        slot_text += f"⭐ {row['slot_number']:02d}. {name}\n"

    message = header + slot_text
    if len(message) > 2000:
        raise ValueError("The raffle board is too long for one Discord message.")
    return message


async def refresh_raffle_board():
    raffle = get_raffle()
    if not raffle or not raffle["board_channel_id"] or not raffle["board_message_id"]:
        return False

    try:
        channel = bot.get_channel(raffle["board_channel_id"])
        if channel is None:
            channel = await bot.fetch_channel(raffle["board_channel_id"])
        message = await channel.fetch_message(raffle["board_message_id"])
        await message.edit(content=build_board_message())
        return True
...
class RaffleBot(commands.Bot):
    def __init__(self):
        ...
        self.commands_synced_to_guilds = False
...
async def on_ready():
    ...
    if not bot.commands_synced_to_guilds:
        for guild in bot.guilds:
            bot.tree.copy_global_to(guild=guild)
            await bot.tree.sync(guild=guild)
            print(f"Commands synced to {guild.name} (ID: {guild.id})")
        bot.commands_synced_to_guilds = True
...
async def raffle_create(
    ...
    with db() as conn:
        ...
        conn.execute(
            "INSERT INTO raffle (id, title, prize, slot_count, entry_fee, active, board_channel_id, board_message_id) VALUES (1, ?, ?, ?, ?, 1, NULL, NULL)",
    ...
    try:
        board = build_board_message()
    except ValueError:
        return await interaction.response.send_message(
            "That raffle has too many slots to fit in one live board. Please create a smaller raffle.",
            ephemeral=True,
        )
    ...
    with db() as conn:
        conn.execute(
            "UPDATE raffle SET board_channel_id = ?, board_message_id = ? WHERE id = 1",
            (interaction.channel_id, board_message.id),
        )


@bot.tree.command(name="claim", description="Claim an available raffle slot.")
@app_commands.describe(
    slot="Slot number to claim",
    member="Admin only: select the Discord member claiming this slot",
)
async def claim(
    interaction: discord.Interaction,
    slot: int,
    ...
    if member is not None and member.id != interaction.user.id and not is_admin(interaction):
        return await interaction.response.send_message(
            "Only an admin can claim a slot for another member.", ephemeral=True
        )
    ...
    with db() as conn:
        ...
        conn.execute(
            ...
            (target.id, target.display_name, slot),
        )

    await interaction.response.send_message(
        f"🎟️ {target.mention} claimed **slot #{slot}**.\n"
        f"Status: **UNPAID** — an admin must mark it paid.",
        ephemeral=True,
    )
    await refresh_raffle_board()
...
async def unclaim(interaction: discord.Interaction, slot: int):
    ...
    await interaction.response.send_message(f"♻️ Slot **#{slot}** is open again.", ephemeral=True)
    await refresh_raffle_board()
...
async def mark_paid(interaction: discord.Interaction, slot: int):
    ...
    await interaction.response.send_message(f"✅ Slot **#{slot}** marked **PAID**.", ephemeral=True)
    await refresh_raffle_board()
...
async def mark_unpaid(interaction: discord.Interaction, slot: int):
    ...
    await interaction.response.send_message(f"🟡 Slot **#{slot}** marked **UNPAID**.", ephemeral=True)
    await refresh_raffle_board()
...
async def slots(interaction: discord.Interaction):
    ...
    try:
        message = build_board_message()
    except ValueError:
        return await interaction.response.send_message(
            "This raffle is too long to display in one Discord message.", ephemeral=True
        )

    await interaction.response.send_message(message, ephemeral=True)


@bot.tree.command(name="raffle_board", description="Admin: post a new live-updating raffle board.")
async def raffle_board(interaction: discord.Interaction):
    if not is_admin(interaction):
        return await interaction.response.send_message(
    ...
    if not raffle:
        return await interaction.response.send_message(
            "No raffle has been created.", ephemeral=True
        )
